...
describe('Observability Module Integration & SLA Tests', () => {
  ...
  describe('Target 1: Agent Pipeline Span Coverage', () => {
    it('should verify 100% of LangGraph nodes emit an OpenTelemetry span during execution', async () => {
      ...
      await executeCreativePipeline(brandDnaId, 'Observability campaign test', forceSequence);
  ...
  describe('Target 2: BullMQ Event Hook Coverage', () => {
    it('should verify completed, failed, and stalled lifecycle hooks fire accurately', async () => {
      const testQueue = new BullMQQueueManager('test-hooks-queue');

      // 1. Completed Job
      const job1 = await testQueue.add('job_success', { data: 'ok' });
      await testQueue.processJob(job1.id, async () => ({ status: 'done' }));

      // 2. Failed Job
      const job2 = await testQueue.add('job_fail', { data: 'error' });
      try {
        await testQueue.processJob(job2.id, async () => {
      ...
      // 3. Stalled Job
      const job3 = await testQueue.add('job_stall', { simulateStall: true });
      await testQueue.processJob(job3.id, async () => {});
